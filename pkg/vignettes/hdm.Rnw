...
The \texttt{hdm} packages provides methods for multiple testing adjustment as described in \cite{siminf2018}. Various methods to conduct valid simultaneous inference on a set of target coefficients are implemented in the S3 method \texttt{p\_adjust()}. For example, consider the problem of simultaneously testing 100 target coefficients in a simulated data example. For instance, it is possible to take dependencies among covariates into account by using the stepdown procedure of \cite{romanowolf2005}.
<<siminf>>=
library(mvtnorm) 
set.seed(1)
n = 100
...
# Romano-Wolf Stepdown Correction
p.bh = p_adjust(rl, method = "BH", B = 1000)
# Number of rejections at a prespecified significance level
sum(p.bh[,2]<0.05)
@

...
###
ts <- tsls(y=y,d=d,x=x,z=z, intercept=TRUE)
iv <- ivreg(y~d+x|z+x)
#
...
output3a <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = FALSE, intercept=FALSE, post=FALSE)
output3b <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = FALSE, intercept=FALSE, post=TRUE)
output3c <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = FALSE, intercept=TRUE, post=FALSE)
output3d <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = FALSE, intercept=TRUE, post=TRUE)
output3e <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = TRUE, intercept=FALSE, post=FALSE)
output3f <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = TRUE, intercept=FALSE, post=TRUE)
output3g <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = TRUE, intercept=TRUE, post=FALSE)
output3h <- rlassoLATE(x,d,y,z, bootstrap="none", nRep=500, normalize = TRUE, intercept=TRUE, post=TRUE)
...
################
rm(list=ls())
data_int <- readMat("E:\\R Package hdm\\Misc\\LATE_spec2a.mat")
y <- data_int$y
d <- data_int$d
z <- data_int$z
xS <- data_int$xS
